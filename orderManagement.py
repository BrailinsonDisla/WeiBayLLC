...
# Submits an order.
def submitOrder(productList : tuple, bid = False, bidder_id = None, bid_amount = None):
    ...
    if not anonymous():
        ...
        try:
            ...
            dbCursor.execute("SELECT SUM(Price) FROM ListedProducts WHERE `Product ID` IN {}".format(productList))
...
# Submits a bid.
def submitBid(prod_id : int, bid_amount: float):
    ...
    if not anonymous():
        # Sets the bidder ID.
        bidder_id = 1 #current_user.user_id bidder_id + int(time() % 100000)
        ...
        try:
            ...
            if dbCursor.rowcount == 1:
                ...
                # Creates a tuple with the values to insert.
                bid_data = (bid_id, bidder_id, prod_id, bid_amount, date)

                print(bid_id, time.strftime('%Y%m%d%H%M%S'))
                # Submits the user's bid.
                dbCursor.execute("INSERT INTO Bids (`Bid ID`, `Bidder ID`, `Product ID`, `Bid Amount`, `Bid Date`) "
                                 "VALUES (%s, %s, %s, %s, %s)", bid_data)
    ...
    if not anonymous():
        try:
            ...
            if dbCursor.rowcount == 1:
                ...
                # Sets the bid amount.
                bid_amount = bid_data[3]

                # Submits the bid as an order.
                submitOrder((prod_id), bid = True, bidder_id=bidder_id, bid_amount=bid_amount)
                ...
                dbCursor.execute("INSERT INTO ApprovedBids (`Bid ID`, `Bidder ID`, `Product ID`, `Bid Amount`, `Bid Date`) "
                                 "VALUES (%s, %s, %s, %s, %s)", bid_data)
        ...
        # Catches other, system unrelated, exception.
            ...
            # TODO: Inform the seller that the bid was not proccessed.
